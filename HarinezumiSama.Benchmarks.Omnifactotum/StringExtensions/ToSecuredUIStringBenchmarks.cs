...
public abstract class ToSecuredUIStringBenchmarks(int length)
{
    ...
    public string F0_Initial()
        => Implementation.F0_Initial.ToSecuredUIString(InputValue, Implementation.DefaultLoggedPartLength, Implementation.DefaultMinimumSecuredPartLength);

    [Benchmark]
    public string F1_New()
        => Implementation.F1_New.ToSecuredUIString(InputValue, Implementation.DefaultLoggedPartLength, Implementation.DefaultMinimumSecuredPartLength);

    [SuppressMessage("ReSharper", "InconsistentNaming")]
    private static class Implementation
    {
        internal const int DefaultLoggedPartLength = 4;
        internal const int DefaultMinimumSecuredPartLength = 16;
        ...
        internal static class F0_Initial
        {
                ...
                int minimumSecuredPartLength)
            {
                ...
                var result = value.Length >= minimumLoggedValueLength
                    ? ToUIStringBenchmarks.Implementation.F1_UsingStackOrHeapAllocationAndOnePreSearch.ToUIString(
        ...
        internal static class F1_New
        {
                ...
                int minimumSecuredPartLength)
            {
                ...
                {
                    return ToUIStringBenchmarks.Implementation.F1_UsingStackOrHeapAllocationAndOnePreSearch.ToUIString(
                        $"{value.Substring(0, loggedPartLength)}...{value.Substring(value.Length - loggedPartLength)}");
