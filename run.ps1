...
process
{
    ...
    [ValidateNotNullOrEmpty()] [string] $executableFilePath = `
        ".out/bin/$executableProjectName/AnyCPU/Release/net9.0/$executableProjectName.exe" | Get-ProjectItemFullPath
    ...
    dotnet test --verbosity normal """$solutionFilePath""" --no-build 2>&1
    Test-ExitCode
